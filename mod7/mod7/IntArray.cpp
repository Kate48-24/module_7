...
void IntArray::erase()                          //удалит массив и установит его длинну 0
{
    delete[]m_data;
    m_data = nullptr;
    m_length = 0;
...
void IntArray::reallocate(int newLength)        //при изменении размера массива уничтожит все существующие в нём элементы
{
    erase();
...
void IntArray::resize(int newLength)            //при изменении размера массива сохранит все существующие в нём элементы
{
...
void IntArray::insertBefore(int value, int index)           //вставка
{
    if ((index < 0) || (index > m_length))
    {
        throw Bad_range();
...
void IntArray::remove(int index)                             //удаление
{
    if ((index < 0) || (index >= m_length))
    {
        throw Bad_range();
